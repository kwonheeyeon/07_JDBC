...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		// == DBeaver에서 사용하는 DB 연결(새 데이터 베이스 연결)과 같은 역할의 객체
		...
		try {
			...
			String port = ":1521"; // 프로그램 연결을 위한 구분 번호(포트 번호)
			
			String dbName = ":XE"; // DBMS 이름(XE == express edition)
			
			String userName = "KH_KHY"; // 사용자 계정명
			
			String password = "KH1234"; // 계정 비밀번호
			
			/* 2-3) DB 연결 정보와 DriverManger를 이용해서 Connection 객체 생성 */
...
//			System.out.println(conn);
			 ...
			 * 	  1행씩 접근해 각 행의 작성된 컬럼 값 얻어오기 */
			
			// rs.next() : 커서를 다음 행으로 이동 시킨 후 이동된 행에 값이 있으면 true, 없으면 false 반환
			// 			   맨 처음 호출 시 1행부터 시작
			while(rs.next()) {
